<script lang="ts">
	...
	const handleInput = async (event: Event & { currentTarget: EventTarget & HTMLInputElement; }) => {
        isLoading = true;
...
<div class="autocomplete">
    <input class="autocomplete-input" bind:value={searchText} on:input={handleInput} on:keydown={handleKeydown}/>
        ...
        {:else}
            {#if countries.length !== 0}
                {#each countries as country}
                    <AutocompleteListItem name={country.name} level={123} rank="Silver 1" tier={11} searchText={searchText}/>
                {/each}
            {/if}
        {/if}
    </div>
