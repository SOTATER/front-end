<script>
  ...
  const handleClick = (selectedTab) => () => (activeTab = selectedTab);
</script>

<ul>
  {#each items as item, i}
    <li
      class={activeTab === item.activeOption
        ? `active index-${i}`
        : `non-active index-${i}`}
    >
...
<style>
  ...
  .index-2 > .tab > .icon {
    ...
    height: 17px;
    margin-right: 2px;
    background: url('https://opgg-static.akamaized.net/images/plus/icon-ingame-n.png')
      no-repeat;
